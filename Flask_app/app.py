...
def upload():
    ...
    try:
        ...
        desc["ML"] = "Detect the ransomware by machine learning using random forest with a datasets."
    # obsufuctions
    data["Entropy"],data["Encryption Algorithmes"],data["Anti debugging detection"],data["Anti vms detection"] = Obsufuctions_Analysis()
    ...
    # Behaviour analysis
    data["Behaviour Detection"], desc["Behaviour_Detection"], familly = behav_analysis()
    # Solution deja existe
    ss = []
    data["Intezer API"], data["Scanii API"], ss = solution_deja_existante()
    try :
        if len(ss) == 2:
    ...
    # Generate the report
    path_to_pdf = generate_report(data,s,familly)

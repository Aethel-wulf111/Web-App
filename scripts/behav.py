...
def Initial_Connection(hash_file):
    ...
    files = {'file': (open(load_and_read_exe(), 'rb'))}
...
def behav_analysis(filename):
    ...
    with open(json_file) as f:
        report_data = f.read()
    dictionary = json.loads(report_data)
    Initial_Connection(dictionary[exe_file]["md5Hash"])
    ...
    if not Exeception_length(json_file):
        if not Exception_Verdicts(json_file):
            ...
            print("The ransomware famillie of the file is "+str(max))
            return False, "The ransomware famillie of the file is "+str(max), max
        else: # Pour eviter les faux positives
            print("["+str(now)+"]~ The behaviour of the file has been done successful!")
            #print("Exception has been handle it, The file probably be clean")
        ...
        if "error" in dictionary:
            ...
            print("["+str(now)+"]~ The behaviour of the file has been done successful!")
            #print("Exception has been handle it, we can not found the file for the behaviour analysis OR the file probably be clean")
